# 🚀 PROJETO 3V2 — SISTEMA DE EMISSÃO DE CARTÕES BANCÁRIOS

# 🎯 PROBLEMA

Você faz parte da equipe de desenvolvimento de um banco que emite cartões bancários de tipos diferentes.

Atualmente o sistema precisa lidar com três situações diferentes:

- Cartão de débito emitido;
- Cartão de crédito emitido;
- Cartão múltiplo emitido.
...
### RF01 — Cartão de débito emitido

O sistema deve permitir criar cartão de débito emitido.

O objeto deve possuir titular e, quando a operação for executada, deve produzir:

```
Cartão de débito emitido para: TITULAR
```

Exemplo:

```
Cartão de débito emitido para: Marina Cavalcante
```

### RF02 — Cartão de crédito emitido

O sistema deve permitir criar cartão de crédito emitido.

Quando a operação for executada, deve produzir:

```
Cartão de crédito emitido para: TITULAR
```

Exemplo:

```
Cartão de crédito emitido para: Otávio Ramos
```

### RF03 — Cartão múltiplo emitido

O sistema deve permitir criar cartão múltiplo emitido.

Quando a operação for executada, deve produzir:

```
Cartão múltiplo emitido para: TITULAR
```

Exemplo:

```
Cartão múltiplo emitido para: Helena Brandão
```
...
Todos os tipos criados devem ser considerados um `Cartao`.
...
```
criar("debito", "Marina Cavalcante")
```
...
Caso seja solicitado um tipo que o sistema não conhece, como `pre-pago-internacional`, a criação deverá gerar um erro:
...
- Escreva o seu código nos arquivos `Cartao.js` e `Factory.js` (você precisa criá-los);
- os textos de saída devem ser exatamente iguais aos apresentados nos requisitos;
- não altere os arquivos da pasta `test`.

# ▶️ COMO EXECUTAR O PROJETO

Dentro da sua pasta (`Eryckgk`), instale as dependências:
